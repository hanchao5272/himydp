...
设计原则：要依赖抽象，不要依赖具体类==》依赖导致原则
### 4.4.抽象工厂模式，以确保披萨原料一致为例
抽象工厂模式:提供一个接口，用于创建相关或依赖对象的家族，而不需要指定明确的类。
抽象工厂模式允许客户使用抽象的接口来创建一组相关的产品，而不需要知道实际产出的产品是什么。
抽象工厂模式要素：
1. AbstractFactory（抽象工厂接口）：包含一组方法用于生成产品，所有的具体工厂都必须实现这个接口，并重写生成产品的接口。
2. Client(客户类)：客户类中只需设计抽象工厂，运行时自动使用实际的工厂。
3. AbstractProduct(抽象产品接口):定义了一种产品的接口，通常有一组抽象产品接口。
4. ConcreteFactory（具体工厂类）：每个具体工厂类实现了抽象工厂接口，都能够实现不同的产品家族。要创建一个产品，客户不需要实例化任何
   产品类，只需要使用其中一个工厂方法即可。
5. ConcreteProduct(具体产品类)：每个抽象产品接口都对应一组具体产品类。

抽象工厂模式与工厂方法模式
1. 抽象工厂的方法经常以工厂方法的模式实现
2. 抽象工厂和工厂方法都负责封装对象创建，都实现了解耦。
3. 工厂方法时通过继承类实现的解耦：利用工厂方法创建对象，需要扩展一个类，并覆盖它的工厂方法。
4. 抽象工厂是通过组合对象实现的解耦：提供一个用于创建一个产品家族的抽象类型，这个类的子类定义了产品被产生的方法。要想使用
   这个工厂，必须先实例化它，然后将它传入一些针对抽象类型所写的代码中。
   
设计原则：依赖抽象，不要依赖具体类（依赖倒转原则）
